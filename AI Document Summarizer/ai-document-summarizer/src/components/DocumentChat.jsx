...
// ── Main component ────────────────────────────────────────────────────────────
function DocumentChat({ documentId, initialChatHistory = [] }) {
  ...
  async function loadChatHistory() {
    ...
    try {
      ...
      const res = await api.get(`/api/history/${documentId}/chat`);
  ...
  async function handleAsk() {
    ...
    try {
      const res = await api.post(`/api/history/${documentId}/chat`, { question: q });
      ...
      setMessages(prev => [
        ...prev,
        { role: "assistant", text: "Sorry, something went wrong answering that. Please try again.", createdAt: new Date() }
      ]);
    } finally {
      setAsking(false);
    }
          ...
          <div className="max-h-96 overflow-y-auto p-5 space-y-4">
            {loadingHistory ? (
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-6">
                Ask a question about this document — e.g. "What are the deadlines mentioned?" or "Summarize the risks."
              </p>
            ) : (
              messages.map((m, i) => (
                <div
